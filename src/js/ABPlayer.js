...
let parseXmlSafe = function parseXmlSafe(text) {
    'use strict';
...
function CommentLoader(url, xcm, callback) {
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
            let standarizedXML = xmlhttp.responseXML === null ? parseXmlSafe(xmlhttp.responseText) : xmlhttp.responseXML;
...
Element.prototype.addClass = function() {
    return this.classList.add(...arguments);
};
Element.prototype.removeClass = function() {
    return this.classList.remove(...arguments);
};
...
(function() {
    ...
    let findClosest = function(node, className) {
    ...
    let formatInt = function(source, length) {
        let strTemp = '';
    ...
    let formatDate = function(timestamp, shortFormat) {
        if (timestamp === 0) {
            return '';
        }
    ...
    let convertTime = function(formattedTime) {
        let timeParts = formattedTime.split(':'),
            total = 0;
        for (let i = 0; i < timeParts.length; i++) {
            total *= 60;
            let value = parseInt(timeParts[i]);
            if (isNaN(value) || value < 0) return false;
            total += value;
        }
        return total;
    };
    let hoverTooltip = function(elem, follow, yoffset) {
        if (!elem) return;
        elem.addEventListener('mousemove', function(e) {
            let tooltip = $('ABP-Tooltip'),
    ...
    let buildFromDefaults = function(n, d) {
        let r = {};
        for (let i in d) {
    ...
    ABP.bind = function(playerUnit, state) {
        var ABPInst = {
            ...
            removePopup: function() {
                let pops = playerUnit.getElementsByClassName('ABP-Popup');
            ...
            loadCommentList: function(sort, order) {
                order = order === 'asc' ? -1 : 1;
                let keysSorted = Object.keys(ABPInst.commentList).sort(function(a, b) {
                ...
                for (let i in keysSorted) {
                    ...
                    let comment = ABPInst.commentList[key];
            ...
            renderCommentList: function() {
                let offset = ABPInst.commentListContainer.parentElement.scrollTop,
                ...
                for (var i = firstIndex; i <= firstIndex + 40; i++) {
                    if (typeof ABPInst.commentObjArray[i] !== 'undefined') {
                        var commentObj;
                        if (i === firstIndex && i > 0) {
                            commentObj = ABPInst.commentObjArray[i].cloneNode(true);
                            let commentObjContent = commentObj.getElementsByClassName('cmt-content')[0],
                            ...
                            commentObj = ABPInst.commentObjArray[i];
                        }
        ...
        ABPInst.swapVideo = function(video) {
            ...
            if (ABPInst.cmManager) {
                ABPInst.cmManager.addEventListener('load', function() {
                    ABPInst.commentList = {};
                    for (let i in ABPInst.cmManager.timeline) {
                        let danmaku = ABPInst.cmManager.timeline[i];
                ...
                ABPInst.cmManager.setBounds = function() {
                    ...
                    let actualWidth = ABPInst.videoDiv.offsetWidth,
                    ...
                    for (let a in this.csa) this.csa[a].setBounds(this.width, this.height);
        ...
        let _v = playerUnit.getElementsByClassName('ABP-Video');
        if (_v.length <= 0) return;
        var video = null;
        ABPInst.videoDiv = _v[0];
        for (let i in _v[0].children) {
        ...
        let cmtc = _v[0].getElementsByClassName('ABP-Container');
        if (cmtc.length > 0)
            ABPInst.divComment = cmtc[0];
        if (video === null) return;
        ABPInst.video = video;
        /** Bind the Play Button **/
        let _p = playerUnit.getElementsByClassName('ABP-Play');
        ...
        let pbar = playerUnit.getElementsByClassName('progress-bar');
        if (pbar.length <= 0) return;
        let pbars = pbar[0].getElementsByClassName('bar');
        ...
        let fbtn = playerUnit.getElementsByClassName('ABP-FullScreen');
        ...
        let wfbtn = playerUnit.getElementsByClassName('ABP-Web-FullScreen');
        ...
        let wsbtn = playerUnit.getElementsByClassName('ABP-WideScreen');
        ...
        let cfbtn = playerUnit.getElementsByClassName('ABP-Comment-Font');
        ...
        $$('.ABP-Comment-List-Title *').forEach((el) => el.onclick = function(e) {
            let item = e.target.attr('item'),
                order = e.target.hasClass('asc') ? 'desc' : 'asc';
            $$('.ABP-Comment-List-Title *').forEach((el) => el.removeClass('asc') || el.removeClass('desc'));
            e.target.addClass(order);
            ABPInst.loadCommentList(item, order);
        });
        $$('.ABP-Unit .ABP-CommentStyle .ABP-Comment-FontOption .style-option').forEach((el) => el.onclick = function(e) {
            e.target.parentNode.childNodes.forEach((elem) => elem.removeClass('on'));
            e.target.addClass('on');
            ABPInst[e.target.parentNode.attr('name')] = parseInt(e.target.attr('value'));
        });
        colorPick(playerUnit.find('.ABP-Comment-ColorPicker'), function(hex) {
            ABPInst.commentColor = hex.slice(1);
        ...
        if (video.isBound !== true) {
            ...
            ABPInst.btnVolume.addEventListener('click', function() {
                if (ABPInst.video.muted === false) {
                    ...
                    if (ABPInst.video.volume < 0.10) this.className += 'mute';
                    else if (ABPInst.video.volume < 0.33) this.className += 'low';
                    else if (ABPInst.video.volume < 0.67) this.className += 'medium';
            ...
            ABPInst.timeLabel.addEventListener('click', function() {
                ...
                ABPInst.timeJump.addEventListener('keydown', function(e) {
                    if (e.keyCode === 13) {
                        let time = convertTime(ABPInst.timeJump.value);
            ...
            document.addEventListener('mouseup', function(e) {
                if (dragging) {
                    let newTime = ((e.clientX - ABPInst.barTimeHitArea.getBoundingClientRect().left) / ABPInst.barTimeHitArea.offsetWidth) * ABPInst.video.duration;
            ...
            let updateVolume = function(volume) {
                ...
                if (volume < 0.10) ABPInst.btnVolume.className += 'mute';
                else if (volume < 0.33) ABPInst.btnVolume.className += 'low';
                else if (volume < 0.67) ABPInst.btnVolume.className += 'medium';
            ...
            document.addEventListener('mouseup', function(e) {
                if (draggingVolume) {
                    let newVolume = (e.clientX - ABPInst.barVolumeHitArea.getBoundingClientRect().left) / ABPInst.barVolumeHitArea.offsetWidth;
            ...
            document.addEventListener('mouseup', function(e) {
                if (draggingOpacity) {
                    let newOpacity = (e.clientX - ABPInst.barOpacityHitArea.getBoundingClientRect().left) / ABPInst.barOpacityHitArea.offsetWidth;
            ...
            let updateScale = function(scale) {
            ...
            document.addEventListener('mouseup', function(e) {
                if (draggingScale) {
                    let newScale = 0.2 + 4.8 * (e.clientX - ABPInst.barScaleHitArea.getBoundingClientRect().left) / ABPInst.barScaleHitArea.offsetWidth;
            ...
            playerUnit.addEventListener('keydown', function(e) {
                if (e && document.activeElement.tagName !== 'INPUT') {
                    ...
                    case 32:
                        ...
                        break;
                    case 37:
                        newTime = ABPInst.video.currentTime -= 5;
                        ABPInst.cmManager.clear();
                        if (newTime < 0) newTime = 0;
                    ...
                    case 39:
                        ...
                        break;
                    case 38:
                        newVolume = ABPInst.video.volume + 0.1;
                        if (newVolume > 1) newVolume = 1;
            ...
            playerUnit.addEventListener('touchend', function(e) {
                if (e.changedTouches.length > 0) {
                    if (_touch !== null) {
                        let diffx = e.changedTouches[0].pageX - _touch.pageX;
                        let diffy = e.changedTouches[0].pageY - _touch.pageY;
        ...
        if (typeof CommentManager !== 'undefined') {
            if (ABPInst.state.autosize) {
                let autosize = function() {
                    ...
                    let boundW = playerUnit.offsetWidth;
                    let boundH = playerUnit.offsetHeight;
                    let oldASR = boundH / boundW;
